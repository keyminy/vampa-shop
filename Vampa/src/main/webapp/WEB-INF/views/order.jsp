				...
				<div class="content_main">
					...
					<div class="addressInfo_div">
						<div class="addressInfo_button_div">
							<button class="address_btn address_btn1" style="background-color: #3c3838;"
							onclick="showAdress('1')">사용자 정보 주소록</button>
							<button class="address_btn address_btn2"
...
$(".order_point_input").on("propertychange change keyup paste input",function(){
	...
	if(inputValue<0){
		//0보다 작은 값 입력하면 0으로 셋팅
		$(this).val(0);
	}else if(inputValue > maxPoint){
		//가지고 있는 포인트보다 많은 값을 입력할 경우, 사용가능 포인트 최댓값이 입력되도록
		$(this).val(maxPoint);
	}
	/* 주문 종합 정보란 셋팅 */
	setTotalInfo();
 ...
 */
 ...
 /* 총 주문 종합 정보 셋팅(배송비,총 가격,마일리지,물품 수,종류) */
 ...
 $(".order_btn").on("click",function(){
	 /*배송지 정보가 form태그의 <input>에 저장되도록 하기 */
	 /* 주소 정보 & 받는이 */
	 $(".addressInfo_input_div").each(function(i,obj){
		 ...
		 /* 회원이 입력한 사용 포인트를 <input>태그에 저장 */
		 $("input[name='usePoint']").val($(".order_point_input").val());
		 
		 /* 버튼 클릭 시,상품정보 <input>동적 생성 */
		 let form_contents = '';
		 $(".goods_table_price_td").each(function(index, element){
