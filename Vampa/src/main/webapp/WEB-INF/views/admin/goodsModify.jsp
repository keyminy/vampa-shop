...
<link rel="stylesheet" href="/css/admin/goodsModify.css">
<link rel="stylesheet"
...
<body>
	...
	<script>
		$(document).ready(function(){
			...
			$(function(){
				...
				//datePicker를 적용시키는 코드에 DB에 저장된 데이터가 출력되게 하기
	...
	<script>
		//사용자 선택에 따라 카테고리 분류들이 변경되도록 하기
		//렌더링 될 때 실행될 코드와 구분하기 위해 새로운 script 추가
	/* 카테고리 */
	let cateList = JSON.parse('${cateList}');
	...
	$("input[name='bookPrice']").on("change",function(){
		...
		let sendDiscountRate = discountRate/100;//서버에 전송할 할인값
		let goodsPrice = $("input[name='bookPrice']").val();//원가
		let discountPrice = goodsPrice * (1 - sendDiscountRate); //할인 적용된 가격
