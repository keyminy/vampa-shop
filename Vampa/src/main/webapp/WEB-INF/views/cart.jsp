...
<body>
	...
	<script>
		...
		/* 체크박스의 변환에 따라서 실행되는 메서드 추가 */
		
		/* 체크 여부에따른 종합 정보 변화 */
		$(".individual_cart_checkbox").on("change",function(){
			/* 체크박스 변경되면, '종합 정보 섹션' 정보들이 업데이트 되도록 */
			setTotalInfo();
		});
		...
		$(".all_check_input").on("click", function(){
			if($(".all_check_input").prop("checked")){
				//prop으로인한 속성이 checked이면 각각의 checkbox를 checked로 만들기
		...
		/*그 전에,'종합 정보 섹션'의 데이터를 만들어내던 코드를 메서드로 추출 */
		/* setTotalInfo() : 총 주문 정보 셋팅(배송비,총 가격,총 마일리지,물품 수,종류)*/
		function setTotalInfo(){
			...
			/* 배송비 결정 : 30000원 이상이면 0원,그 이하는 3000원 */
			if(totalPrice >= 30000){
				deliveryPrice=0;
		...
		$(".quantity_modify_btn").on("click", function(){
				...
				/* form태그 내부에 작성된 input태그에 저장 후 전송 */
		...
		$(".order_btn").on("click",function(){
			// form_contents : 동적으로 생성할 <input>태그 문자열 값이 저장됨
			let form_contents = '';
			let orderNumber = 0; //orders[0],orders[1]...과 같은 index값
			$(".cart_info_td").each(function(index,element){
				//check되어있는 상품만 주문되어야 하므로 if조건
				if($(element).find(".individual_cart_checkbox").is(":checked")===true){
					...
					//orderNumber는 다음 객체에 접근 할 때 +1이 된다
					orderNumber += 1;
				}
			});//end each
			//form태그 내부에 저장된 문자열 추가
